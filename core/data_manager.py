...
class DataManager:
    ...
    def add_to_group_whitelist(self, group_id: str) -> bool:
        """添加群到白名单"""
        whitelist = self.global_config.get('group_whitelist', [])
        if group_id not in whitelist:
            whitelist.append(group_id)
            self.global_config['group_whitelist'] = whitelist
            return self._save_global_config()
        return False
    
    def remove_from_group_whitelist(self, group_id: str) -> bool:
        """从白名单移除群"""
        whitelist = self.global_config.get('group_whitelist', [])
    ...
    def get_group_whitelist(self) -> list:
        """获取群白名单列表"""
        return self.global_config.get('group_whitelist', [])
    
    # ===== 过滤群管理 =====
    
    def add_to_filtered_groups(self, group_id: str) -> bool:
        """添加群到过滤列表"""
        filtered = self.global_config.get('filtered_groups', [])
        if group_id not in filtered:
            filtered.append(group_id)
            self.global_config['filtered_groups'] = filtered
            return self._save_global_config()
        return False
    
    def remove_from_filtered_groups(self, group_id: str) -> bool:
        """从过滤列表移除群"""
        filtered = self.global_config.get('filtered_groups', [])
    ...
    def is_group_filtered(self, group_id: str) -> bool:
        """检查群是否在过滤列表中"""
        filtered = self.global_config.get('filtered_groups', [])
        return group_id in filtered
    
    def get_filtered_groups(self) -> list:
        """获取过滤群列表"""
        return self.global_config.get('filtered_groups', [])
